<script lang="ts">
	...
	function getIconPath(iconName: string): string {
		const icons = {
			...
			ticket: 'M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 11-2-4V7a2 2 0 00-2-2H5z',
			paint: 'M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zM7 3H5v16c0 1.1.9 2 2 2s2-.9 2-2V3zM15 3v18l4-4V7l-4-4z'
		};
		return icons[iconName] || icons.home;
	}

	function isAdminUser(): boolean {
		return data.user?.Administrator || 
			data.user?.Permissions.CanManageUsers || 
			data.user?.Permissions.CanManageRoles ||
			data.user?.Permissions.CanManageInvites ||
			data.user?.Permissions.CanManageTheme || false;
	}
</script>
...
<div class="min-h-screen bg-gray-50">
	...
	<!-- Sidebar -->
	<div class="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:inset-0 {sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0">
		<div class="flex h-16 items-center justify-between px-6 border-b border-gray-200">
			<div class="flex items-center">
				<div class="h-8 w-8 flex items-center justify-center rounded-lg bg-indigo-600">
		...
		<nav class="mt-6 px-3">
			<div class="space-y-1">
				{#each navigation as item}
					...
					>
						<svg class="mr-3 h-5 w-5 {item.current ? 'text-indigo-500' : 'text-gray-400 group-hover:text-gray-500'}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
	...
	<div class="lg:pl-64">
		<!-- Top bar -->
		<div class="sticky top-0 z-10 bg-white border-b border-gray-200 px-4 sm:px-6 lg:px-8">
