...
<article class="flex w-full flex-col items-start justify-center gap-4 self-stretch rounded-lg border border-muted p-8 text-left">
    <h1 class="w-full text-3xl font-semibold lg:w-[775px] lg:text-6xl">Intern Events & Sessions</h1>
    <p class="text-xl lg:w-[720px]">
        Explore our upcoming tech events, workshops, and presentations. Join us for exciting learning opportunities and networking sessions.
    </p>
                ...
                {#each agenda_data as group}
                    <div class="rounded-lg border border-muted bg-card p-6">
                        ...
                        <div class="flex items-center gap-2 text-muted-foreground">
                            <Calendar size={16} />
                            <span>{group.agenda.StartTime.toLocaleDateString()} - {group.agenda.EndTime.toLocaleDateString()}</span>
                        </div>
                        
                        <div class="mt-6 space-y-4">
                            {#each group.agenda.events as event}
                                <div class="relative flex gap-4 border-l-2 border-primary pl-4 pb-8">
                                    ...
                                    <div class="w-full rounded-lg bg-muted/50 p-4">
                                        ...
                                        <div class="mt-2 flex items-center gap-2 text-sm text-muted-foreground">
                                            <Clock size={14} />
                                            <span>{event.StartTime.toLocaleTimeString()} - {event.EndTime.toLocaleTimeString()}</span>
                                        </div>
